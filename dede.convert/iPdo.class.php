...
defined('iPHP_DB_DRIVER') OR define('iPHP_DB_DRIVER', 'mysql');
...
class iPdo {
    ...
    public function __construct($config=null,$flag='iPHP_DB') {
        extension_loaded('PDO') OR die('您的 PHP 环境看起来缺少 PDO 数据库部分，这对 iPHP 来说是必须的。');
        if(isset($GLOBALS[$this->dbFlag])){
            $this->link = $GLOBALS[$this->dbFlag];
            if($this->link){
                if($this->link->ping())
                    return $this->link;
            }
    ...
    public function connect($flag=null) {
        ...
        try {
            $this->link = new PDO ($this->config['DRIVER'].':host='.$this->config['HOST']
            ...
            echo $e->getMessage ();
            $this->bail("<h1>数据库连接失败</h1><p>请检查 <em><strong>config.php</strong></em> 的配置是否正确!</p><ul><li>请确认主机支持MySQL?</li><li>请确认用户名和密码正确?</li><li>请确认主机名正确?(一般为localhost)</li></ul><p>如果你不确定这些情况,请询问你的主机提供商.如果你还需要帮助你可以随时浏览 <a href='http://www.iiiphp.com'>iPHP 支持论坛</a>.</p>");
        }
        if($flag==='link'){
            return $this->link;
        }
    ...
    public function select_db($var=false) {
        $this->link->errorCode() OR $this->bail("<h1>数据库连接失败</h1><p>我们能连接到数据库服务器（即数据库用户名和密码正确） ，但是不能链接到<em><strong> ".iPHP_DB_NAME." </strong></em>数据库.</p><ul><li>你确定<em><strong> ".iPHP_DB_NAME." </strong></em>存在?</li></ul><p>如果你不确定这些情况,请询问你的主机提供商.如果你还需要帮助你可以随时浏览 <a href='http://www.iiiphp.com'>iPHP 支持论坛</a>.</p>");
    }
    // ==================================================================
    ...
    public function query($query,$QT=NULL) {
        ...
        $this->num_queries++;
        SAVEQUERIES && $this->queries[] = array('sql'=>$query, 'exec_time'=>$this->timer_stop());
        ...
        if (in_array($QH,array('INSERT','DELETE','UPDATE','REPLACE','SET','CREATE','DROP','ALTER'))) {
            ...
            $return_val = $result->rowCount();
        } else {

            if($QT=="field") {
                $this->col_info = $result->fetchColumn();
            }else {
                $num_rows = 0;
    ...
    public function get($output = OBJECT) {
        $store = $this->link->query($this->last_query);
        if ( $output == OBJECT ) {
            return $store->fetchObject(PDO::FETCH_ASSOC);
        }else{
...
//      $data = add_magic_quotes($data);
    ...
    public function version() {
        $mysql_version = $this->value('select version()');
