...
class iMysql {
    ...
    public function connect($flag=null) {
        $this->link = new mysqli($this->config['HOST'], $this->config['USER'], $this->config['PASSWORD'],null,$this->config['PORT']);
        if($flag==='link'){
            return $this->link;
        }
        $this->link->connect_errno && $this->bail("<h1>数据库连接失败</h1><p>请检查 <em><strong>config.php</strong></em> 的配置是否正确!</p><ul><li>请确认主机支持MySQL?</li><li>请确认用户名和密码正确?</li><li>请确认主机名正确?(一般为localhost)</li></ul><p>如果你不确定这些情况,请询问你的主机提供商.如果你还需要帮助你可以随时浏览 <a href='http://www.iiiphp.com'>iPHP 支持论坛</a>.</p>");
    ...
    public function select_db($var=false) {
        $sel = $this->link->select_db($this->config['DB']);
        if($var) return $sel;
    ...
    public function query($query,$QT=NULL) {
        ...
        $result = $this->link->real_query($query);
        ...
        if (in_array($QH,array('INSERT','DELETE','UPDATE','REPLACE','SET','CREATE','DROP','ALTER'))) {
            ...
            $return_val = $this->link->affected_rows;
        } else {
            $store = $this->link->store_result();
            if($QT=="field") {
                $this->col_info = $store->fetch_fields();
            }else {
                $num_rows = 0;
                if($store){
...
//      $data = add_magic_quotes($data);
    ...
    public function version() {
        ...
        $mysql_version = preg_replace('|[^0-9\.]|', '', $this->link->server_info);
